...
function capPicks(picks,target){target=Math.floor(Math.max(0,Math.min(5000,num(target,0)))/100)*100;if(target<100)return[];let xs=(Array.isArray(picks)?picks:[]).slice(0,6).map(x=>({...x}));if(!xs.length)return[];const allowed=Math.min(xs.length,Math.floor(target/100));xs=xs.slice(0,allowed);const weights=xs.map(x=>Math.max(0,pickAmount(x)));const weightSum=weights.reduce((a,b)=>a+b,0);const safeWeights=weightSum>0?weights:xs.map(()=>1);const safeSum=safeWeights.reduce((a,b)=>a+b,0);let amounts=xs.map(()=>100),remaining=target-xs.length*100;if(remaining>0){const desired=safeWeights.map(w=>remaining*w/safeSum);for(let i=0;i<amounts.length;i++){const add=Math.floor(desired[i]/100)*100;amounts[i]+=add;remaining-=add}const order=safeWeights.map((w,i)=>({w,i})).sort((a,b)=>b.w-a.w);let oi=0;while(remaining>=100&&order.length){amounts[order[oi%order.length].i]+=100;remaining-=100;oi++}}return xs.map((x,i)=>({...x,stake_yen:amounts[i],amount:amounts[i],stake:amounts[i]}))}
