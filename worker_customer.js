...
async function siteVisibleKeys(date){if(!date)return null;try{const u=new URL(VISIBILITY_API);u.searchParams.set('date',String(date));u.searchParams.set('scope','site');const r=await fetch(u,{headers:{accept:'application/json'},cache:'no-store'});if(!r.ok)return new Set();const d=await r.json();return new Set(Array.isArray(d?.keys)?d.keys:[])}catch{return new Set()}}
async function sourceHistory(params={}){const u=new URL('/api/history',SOURCE_ORIGIN);Object.entries(params).forEach(([k,v])=>v!==undefined&&v!==null&&v!==''&&u.searchParams.set(k,String(v)));const [r,visible]=await Promise.all([fetch(u,{headers:{accept:'application/json'},cache:'no-store'}),siteVisibleKeys(params.date)]);if(!r.ok)throw new Error(`history_${r.status}`);const d=await r.json(),rows=Array.isArray(d?.records)?d.records:[];if(!visible)return rows;return rows.filter(x=>!enter(x)||visible.has(String(x?.race_key||'')))}
