...
function safeRecord(r){if(!r)return null;const p=r.prediction||{},isEnter=enter(r),visibilityCode=r?.__private_enter===true?'FULL_PRIVATE':r?.__sns_private===true?'SNS_PRIVATE':isEnter?'SNS_PUBLIC':null,base={race_date:r.race_date,venue_code:num(r.venue_code),race_no:num(r.race_no),decision:decision(r),stake_total_yen:isEnter?stake(r):0,visibility_code:visibilityCode,customer_visibility_label:visibilityCode==='SNS_PRIVATE'?'サイト限定':null,settlement:isEnter?safeSettlement(r.settlement):null};if(isEnter){const picks=(Array.isArray(r.bets)&&r.bets.length?r.bets:Array.isArray(p.production_picks)?p.production_picks:[]).map(x=>({ticket:x.ticket||x.combination||x.bet||'',stake_yen:num(x.stake_yen??x.amount??x.stake),odds:Number.isFinite(Number(x.odds))?Number(x.odds):undefined,selection_role:x.selection_role||undefined})),tops=(Array.isArray(p?.final_snapshot?.top_combinations)?p.final_snapshot.top_combinations:Array.isArray(p?.top_combinations)?p.top_combinations:[]).slice(0,12),topOdds=tops.map(x=>Number(x?.odds)).filter(x=>Number.isFinite(x)&&x>0),portfolioAvg=topOdds.length?topOdds.reduce((a,b)=>a+b,0)/topOdds.length:null,focusPicks=tops.slice(0,2).map(x=>({ticket:x.ticket||x.combination||'',odds:Number.isFinite(Number(x.odds))?Number(x.odds):undefined,probability:Number.isFinite(Number(x.probability))?Number(x.probability):undefined})).filter(x=>x.ticket);base.prediction={decision:'ENTER',stake_total_yen:stake(r),reason:p.reason||'',selected_theory:p.selected_theory||'',current_theory:p.current_theory||'',strategy:p.strategy||'',strategy_mode:p?.allocation_meta?.strategy_mode||p?.odds_class||'',strategy_version:p?.allocation_meta?.strategy_version||p?.allocation_meta?.method||'',box_lanes:Array.isArray(p?.allocation_meta?.box_lanes)?p.allocation_meta.box_lanes:[],box1_lanes:Array.isArray(p?.allocation_meta?.box1_lanes)?p.allocation_meta.box1_lanes:[],box2_lanes:Array.isArray(p?.allocation_meta?.box2_lanes)?p.allocation_meta.box2_lanes:[],portfolio_avg_top12_odds:Number.isFinite(portfolioAvg)?Number(portfolioAvg.toFixed(4)):null,portfolio_focus_picks:focusPicks,production_picks:picks}}else{const tops=Array.isArray(p?.final_snapshot?.top_combinations)?p.final_snapshot.top_combinations.slice(0,6):[];base.prediction={decision:decision(r),skip_reason:p.skip_reason||p.reason||'',reference_picks:tops.map(x=>({ticket:x.ticket||x.combination||'',probability:Number.isFinite(Number(x.probability))?Number(x.probability):undefined,odds:Number.isFinite(Number(x.odds))?Number(x.odds):undefined})).filter(x=>x.ticket)}}return base}
