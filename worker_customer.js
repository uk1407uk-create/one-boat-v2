...
async function buildOverview(){
  ...
  return{ok:true,date,degraded:!scheduleAvailable||!historyAvailable||!visibility?.available,source:{schedule:scheduleAvailable,predictions:historyAvailable,visibility:visibility?.available===true},active_count:scheduleAvailable?venues.filter(v=>v.state!=='NOEVENT'&&v.state!=='FINISHED').length:null,public_count:historyAvailable?publicItems.length:null,free_limit:freeLimit,free_count:freeCount,free_remaining:freeRemaining,free_note:'正式ENTERが出たレースのみ無料公開',venues,public_items:publicItems}
}
