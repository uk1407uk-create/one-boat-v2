...
async function publicStats(){
  const rows=await sourceHistory({limit:300});
  const today=jstDate();
  const now=dateMs(today);
  const enter=rows.filter(isEnter);
  const todayRows=enter.filter(r=>String(r?.race_date||'').slice(0,10)===today);
  const d7=enter.filter(r=>{const t=dateMs(r?.race_date);return t&&t>=now-6*86400000&&t<=now;});
  const d30=enter.filter(r=>{const t=dateMs(r?.race_date);return t&&t>=now-29*86400000&&t<=now;});
  const latest=enter.filter(r=>r?.settlement).slice().sort((a,b)=>`${b?.race_date||''}-${num(b?.venue_code)}-${num(b?.race_no)}`.localeCompare(`${a?.race_date||''}-${num(a?.venue_code)}-${num(a?.race_no)}`)).slice(0,8).map(settledSummary);
  return {ok:true,updated_at:new Date().toISOString(),today:aggregate(todayRows),days7:aggregate(d7),days30:aggregate(d30),latest};
}
async function publicToday(){
  const today=jstDate();
  const rows=(await sourceHistory({date:today,limit:200})).filter(isEnter).sort((a,b)=>num(a?.venue_code)-num(b?.venue_code)||num(a?.race_no)-num(b?.race_no));
  const items=rows.map(rec=>{
    ...
    return {
      ...
      deadline:p?.deadline_at||p?.deadline||p?.close_time||'', status:s?'SETTLED':'LOCKED',
      settlement:s?settledSummary(rec):null
    };
  });
  return {ok:true,date:today,count:items.length,items};
}
...
export default {
  async fetch(request,env){
    ...
    try{
      if(u.pathname==='/api/health') return json({ok:true,service:'ONE BOAT CUSTOMER',version:'2026-09-16.1'},200,'no-store');
