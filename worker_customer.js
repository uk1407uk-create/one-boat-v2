...
function recordState(rec,closed=false){if(!rec)return closed?'CLOSED':'PENDING';const d=decision(rec);if(d==='PRIVATE_ENTER'||rec?.__private_enter===true)return'PRIVATE';if(enter(rec))return rec.settlement?'SETTLED':'PUBLIC';if(d==='SKIP')return'SKIP';if(d==='WATCH'||d==='FINALIZING')return closed?'CLOSED':'WATCH';return closed?'CLOSED':'PENDING'}
...
async function buildVenue(code){
  ...
  const races=rs.map(sc=>{
    const rec=recBy.get(sc.race_no)||null,closed=scheduleAvailable&&sc.close_min!==null&&sc.close_min<=now;
    const state=historyAvailable?recordState(rec,closed):'UPDATING';
