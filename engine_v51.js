...
export const PRODUCTION_POLICY={
  ...
  candidate_confidence_min:64,
  confidence_gate_enabled:true,
  max_stake_yen:5000,
  per_pick_max_yen:2000,
  max_points:20,
  base_points:4,
  recalc_minutes:10,
  finalize_minutes:5,
...
export const DEFAULT_POLICY={...BASE_POLICY,...AUXILIARY_POLICY,...PRODUCTION_POLICY};
...
export function pointCapForOdds(oddsInput,policy=DEFAULT_POLICY){
  const odds=Number(oddsInput||0),caps=policy.category_point_caps||{};
  if(!(odds>=3))return 0;
  if(odds<20)return Number(caps.steady??4);
  if(odds<40)return Number(caps.mid_20_39??6);
  if(odds<60)return Number(caps.mid_40_59??8);
  if(odds<100)return Number(caps.target_60_99??10);
  if(odds<200)return Number(caps.high_100_199??12);
  return Number(caps.super_200_plus??20);
}
...
function addSignal(out,key,label,rawScore,maxPoints,meta={}){
  ...
  const score=clamp(rawScore),contribution=(score-50)/50*maxPoints;
...
export function auxiliaryContext(r={},strategy='',policy=DEFAULT_POLICY){
  const signals=[],wind=wind14dSignal(r,policy),inBreak=strategyIsInBreak(strategy);
  ...
  const positive=signals.filter(x=>x.supportive).length,negative=signals.filter(x=>x.opposing).length;
...
function hardGuardReason(reason=''){
  return ['3倍未満は本番対象外','欠損データあり','サンプル不足','穴系は1日最大5Rに到達','超高配当は穴スコア確認待ち','超高配当スコア66未満'].includes(reason);
}
...
export function evaluateRace(r={},policy=DEFAULT_POLICY,history={}){
  ...
  const initial=selectInitialTheory(r,dayTheory,policy),selectedTheory=initial.theory;
  const base=baseEvaluateRace({...r,current_theory:selectedTheory},policy,history);
  const baseValue=Number(base.value_score||0),confidence=Number(base.confidence_score||0);
  ...
  const minEnter=Number(policy.min_confidence_score_shadow??70),candidateMin=Number(policy.candidate_confidence_min??64);
  const pointCap=Math.max(0,Math.min(Number(policy.max_points??20),pointCapForOdds(r.odds??r.expected_odds??0,policy)));
  let decision='SKIP',reason='条件不足',stake=0;

  if(hardGuardReason(base.reason)){
    decision=base.decision;reason=base.reason;
  }else if(confidence<candidateMin){
    decision='SKIP';reason='総合評価64未満';
  }else if(confidence<minEnter){
    decision='WATCH';reason='総合評価64〜69：候補表示';
  }else if(valueScore<candidateMin){
    decision='SKIP';reason='反対材料で価値評価64未満';
  }else if(valueScore<Number(policy.min_value_score??70)){
    decision='WATCH';reason='反対材料で価値評価70未満';
  }else if(inputStake<=0){
    decision='WATCH';reason='投資額未確定';
  }else if(pointCap<1){
    decision='SKIP';reason='分類別点数上限なし';
  }else{
    decision='ENTER';reason='総合評価70以上＋価値基準通過';
    const total=Math.floor(Math.min(Number(policy.max_stake_yen??5000),inputStake)/100)*100;
    let points=Math.max(1,Math.min(pointCap,Number(r.points??policy.base_points??4),Math.floor(total/100)));
    const unit=Math.floor(total/points/100)*100;
    stake=unit*points;
  }
  if(decision!=='ENTER')stake=0;

  const opposing=[...base.breakdown.filter(x=>x.score!==null&&Number(x.score)<=40).map(x=>x.label),...aux.signals.filter(x=>x.opposing).map(x=>x.label)];
  return {
    ...base,
    decision,
    reason,
    ...
    candidate:decision==='WATCH'&&confidence>=candidateMin&&confidence<minEnter,
    point_cap:pointCap,
    auxiliary_adjustment:+adjustment.toFixed(2),
    auxiliary_reasons:aux.signals,
    opposing_materials:[...new Set(opposing)],
    ...
    monitor:{...(base.monitor||{}),participation_target_pct:Number(policy.participation_target_pct??40),aux_supportive_count:aux.supportive_count,aux_opposing_count:aux.opposing_count,aux_raw_adjustment:aux.raw_adjustment}
  };
}
